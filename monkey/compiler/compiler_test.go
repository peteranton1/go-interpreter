// Package compiler compiler/compiler_test.go
package compiler
...
func runCompilerTests(t *testing.T, tests []compilerTestCase) {
	...
	for _, tt := range tests {
		...
		err = testConstants(t,
			tt.expectedConstants, bytecode.Constants)
		if err != nil {
...
func testConstants(
	...
	if len(expected) != len(actual) {
		return fmt.Errorf("wrong number of constants. "+
			"want=%d, got=%d", len(expected), len(actual))
	}
	for i, constant := range expected {
		...
		case int:
			...
			if err != nil {
				return fmt.Errorf("constant %d - "+
					"testIntegerObject failed: %s", i, err)
			}
		...
		case string:
			...
			if err != nil {
				return fmt.Errorf("constant %d - "+
					"testStringObject failed: %s", i, err)
			}
		...
		case []code.Instructions:
			...
			if !ok {
				return fmt.Errorf("constant %d - "+
					"not a function: %T", i, actual[i])
			}

			err := testInstructions(constant, fn.Instructions)
			if err != nil {
				return fmt.Errorf("constant %d - "+
					"testInstructions failed: %s", i, err)
			}
		}
	}
	return nil
...
func testStringObject(expected string, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("object has wrong value - "+
			"got=%q, want=%q", result.Value, expected)
	}
	return nil
}
